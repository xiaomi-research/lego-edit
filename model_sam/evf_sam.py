...
class EvfSamModel(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(len(feat)):
            ...
            # print(f"yxt debug pred mask shape: {pred_mask.shape}")
            # print(f"yxt debug pred_mask shape:  {pred_mask[:, 0].shape}")
            # print("1111111111")
    ...
    def inference(
        ...
        # print("output[encoder_out]: ", output["encoder_out"].shape)
        feat = output["encoder_out"][:, :1, ...]
        # print("feat: ", feat.shape)
        feat = self.text_hidden_fcs[0](feat)
        # print("hidden feat: ", feat.shape)
        (
            sparse_embeddings,
            dense_embeddings,
        ) = self.visual_model.prompt_encoder(
